...
spark = SparkSession \
    .builder \
    .appName('spark-bigquery-mia') \
    .getOrCreate()

raw = spark.read.format('bigquery') \
    .load(f'{project_id}.{raw_dataset_name}.mia_objects_table')

summary_details = raw.select(
    col('title'),
    col('classification').alias('type'),
    convert_date_range(col('dated')).alias('date')) \
    .withColumn('museum', lit('Minneapolis Institute of Art'))
